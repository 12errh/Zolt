"""
Zolt Example — Admin Panel (CRUD)

Run with: zolt run examples/admin/app.py
"""
    ...
    def compose(self) -> None:
        ...
        with Flex(direction="col", gap=6).className("py-8"):
            ...
            Table(
                ...
                rows=[
                    [str(u["id"]), u["name"], u["email"], u["role"], u["status"]] for u in _USERS
                ],
            ).striped()
    ...
    def compose(self) -> None:
        ...
        with Flex(direction="col", gap=6).className("max-w-lg py-8"):
            ...
            with Form(title="User Details").className(
                ...
                Input(
                    value=_new_email, placeholder="email@example.com", label="Email", type="email"
                )
