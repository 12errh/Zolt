...
# ── Reactive state for theme switching ────────────────────────────────────────
_current_theme = reactive("light")
...
class StorybookPage(Page):
    ...
    def _sidebar(self) -> None:
        with Flex(direction="col", align="start", gap=0).className(
            ...
            # Logo
            with Flex(align="center", gap=2).className("px-4 py-5 flex-shrink-0"):
    ...
    def _section_header(self, title: str, desc: str, anchor: str) -> None:
        ...
        with (
            ...
            .className("mt-14 mb-5 first:mt-0 sb-section-anchor")
            .id(anchor)
        ):
    ...
    def _card_wrap(self, label: str, hint: str = "") -> Flex:
        ...
        with card, Flex(align="center", justify="between").className("px-5 pt-4 pb-0"):
    ...
    # THEME SELECTOR
    # =========================================================
    def _section_themes(self) -> None:
        self._section_header(
            "Theme Selector",
            "Switch between all 6 built-in themes — light, dark, ocean, sunset, forest, rose.",
            "themes",
        )

        with self._card_wrap("Active Theme Switcher", "Click to change theme"):
            with self._pad():
                Text(lambda: f"Current theme: {_current_theme.get()}").style("muted").paragraph()

                with Grid(cols=3, gap=4):
                    # Light theme
                    with Flex(direction="col", gap=2).className(
                        "p-4 bg-white border-2 rounded-xl cursor-pointer transition-all "
                        "hover:shadow-md hover:-translate-y-0.5 "
                        + (
                            "border-violet-500 shadow-lg"
                            if _current_theme.get() == "light"
                        ...
                        RawHTML(
                            "<div onclick=\"pyuiSetTheme('light')\" "
                            'class="cursor-pointer">'
                            '<div class="flex items-center gap-2 mb-2">'
                            '<div class="w-8 h-8 rounded-lg bg-gradient-to-br from-violet-500 to-purple-600"></div>'
                            '<div class="text-sm font-semibold text-gray-900">Light</div>'
                            "</div>"
                            '<div class="text-xs text-gray-500">Clean, modern default</div>'
                    ...
                    # Dark theme
                    with Flex(direction="col", gap=2).className(
                        "p-4 bg-gray-900 border-2 rounded-xl cursor-pointer transition-all "
                        "hover:shadow-md hover:-translate-y-0.5 "
                        + (
                            "border-violet-400 shadow-lg"
                            if _current_theme.get() == "dark"
                            else "border-gray-700"
                        )
                    ):
                        RawHTML(
                            "<div onclick=\"pyuiSetTheme('dark')\" "
                            'class="cursor-pointer">'
                            '<div class="flex items-center gap-2 mb-2">'
                            '<div class="w-8 h-8 rounded-lg bg-gradient-to-br from-violet-400 to-purple-500"></div>'
                            '<div class="text-sm font-semibold text-gray-100">Dark</div>'
                            "</div>"
                            '<div class="text-xs text-gray-400">Elegant dark mode</div>'
                            '<div class="flex gap-1 mt-3">'
                    ...
                    # Ocean theme
                    with Flex(direction="col", gap=2).className(
                        "p-4 bg-sky-50 border-2 rounded-xl cursor-pointer transition-all "
                        "hover:shadow-md hover:-translate-y-0.5 "
                        + (
                            "border-sky-500 shadow-lg"
                            if _current_theme.get() == "ocean"
                            else "border-sky-200"
                        )
                    ):
                        RawHTML(
                            "<div onclick=\"pyuiSetTheme('ocean')\" "
                            'class="cursor-pointer">'
                            '<div class="flex items-center gap-2 mb-2">'
                            '<div class="w-8 h-8 rounded-lg bg-gradient-to-br from-sky-400 to-blue-500"></div>'
                            '<div class="text-sm font-semibold text-sky-900">Ocean</div>'
                            "</div>"
                            '<div class="text-xs text-sky-700">Calm, professional</div>'
                            '<div class="flex gap-1 mt-3">'
                    ...
                    # Sunset theme
                    with Flex(direction="col", gap=2).className(
                        "p-4 bg-orange-50 border-2 rounded-xl cursor-pointer transition-all "
                        "hover:shadow-md hover:-translate-y-0.5 "
                        + (
                            "border-orange-500 shadow-lg"
                            if _current_theme.get() == "sunset"
                            else "border-orange-200"
                        )
                    ):
                        RawHTML(
                            "<div onclick=\"pyuiSetTheme('sunset')\" "
                            'class="cursor-pointer">'
                            '<div class="flex items-center gap-2 mb-2">'
                            '<div class="w-8 h-8 rounded-lg bg-gradient-to-br from-orange-400 to-red-500"></div>'
                            '<div class="text-sm font-semibold text-orange-900">Sunset</div>'
                            "</div>"
                            '<div class="text-xs text-orange-700">Warm, energetic</div>'
                            '<div class="flex gap-1 mt-3">'
                    ...
                    # Forest theme
                    with Flex(direction="col", gap=2).className(
                        "p-4 bg-emerald-50 border-2 rounded-xl cursor-pointer transition-all "
                        "hover:shadow-md hover:-translate-y-0.5 "
                        + (
                            "border-emerald-500 shadow-lg"
                            if _current_theme.get() == "forest"
                            else "border-emerald-200"
                        )
                    ):
                        RawHTML(
                            "<div onclick=\"pyuiSetTheme('forest')\" "
                            'class="cursor-pointer">'
                            '<div class="flex items-center gap-2 mb-2">'
                            '<div class="w-8 h-8 rounded-lg bg-gradient-to-br from-emerald-400 to-green-500"></div>'
                            '<div class="text-sm font-semibold text-emerald-900">Forest</div>'
                            "</div>"
                            '<div class="text-xs text-emerald-700">Natural, calm</div>'
                            '<div class="flex gap-1 mt-3">'
                    ...
                    # Rose theme
                    with Flex(direction="col", gap=2).className(
                        "p-4 bg-rose-50 border-2 rounded-xl cursor-pointer transition-all "
                        "hover:shadow-md hover:-translate-y-0.5 "
                        + (
                            "border-rose-500 shadow-lg"
                            if _current_theme.get() == "rose"
                            else "border-rose-200"
                        )
                    ):
                        RawHTML(
                            "<div onclick=\"pyuiSetTheme('rose')\" "
                            'class="cursor-pointer">'
                            '<div class="flex items-center gap-2 mb-2">'
                            '<div class="w-8 h-8 rounded-lg bg-gradient-to-br from-rose-400 to-pink-500"></div>'
                            '<div class="text-sm font-semibold text-rose-900">Rose</div>'
                            "</div>"
                            '<div class="text-xs text-rose-700">Bold, expressive</div>'
                            '<div class="flex gap-1 mt-3">'
                            '<div class="w-4 h-4 rounded bg-rose-500"></div>'
                            '<div class="w-4 h-4 rounded bg-rose-100"></div>'
    ...
    def _section_typography(self) -> None:
        self._section_header(
            "Typography", "Headings h1–h6, text variants, and style modifiers.", "typography"
        )
        with Grid(cols=2, gap=5):
            with self._card_wrap("Headings", "h1 – h4"), self._pad():
                Heading("Display Heading", level=1)
                Heading("Section Heading", level=2)
                Heading("Card Title", level=3)
                Heading("Subsection", level=4)
            with self._card_wrap("Text Variants", "6 variants"), self._pad():
    ...
    def _section_badges(self) -> None:
        self._section_header(
            "Badges + Tags", "Status indicators, labels, and categorization chips.", "badges"
        )
        with Grid(cols=2, gap=5):
            with self._card_wrap("Badge", "7 variants"), self._row():
            ...
            with self._card_wrap("Tag", "4 variants + closable"), self._row():
    ...
    def _section_avatars(self) -> None:
        self._section_header(
            "Avatars + Icons", "User representations and the Lucide icon library.", "avatars"
        )
        with Grid(cols=2, gap=5):
            with self._card_wrap("Avatar", "6 sizes"), self._row():
            ...
            with self._card_wrap("Icon", "Lucide CDN"), self._row():
                for name in [
                    "zap",
                    "layers",
                    "cpu",
    ...
    def _section_buttons(self) -> None:
        self._section_header(
            "Buttons", "Every variant, size, and state — the primary action primitive.", "buttons"
        )
        with self._card_wrap("Variants", "7 variants"), self._row():
        ...
        with self._card_wrap("Sizes", "xs → xl"), self._row():
        ...
        with self._card_wrap("States + Icons"), self._row():
    ...
    def _section_inputs(self) -> None:
        self._section_header(
            "Text Inputs", "Input, Textarea, Select, DatePicker, FilePicker.", "inputs"
        )
        with Grid(cols=2, gap=5):
            with self._card_wrap("Input", "4 types"), self._pad():
            ...
            with self._card_wrap("Textarea + Select + Pickers"), self._pad():
    ...
    def _section_controls(self) -> None:
        self._section_header(
            "Controls", "Checkbox, Toggle, and Slider — binary and range inputs.", "controls"
        )
        with Grid(cols=2, gap=5):
            with self._card_wrap("Checkbox + Toggle"), self._pad():
            ...
            with self._card_wrap("Slider", "range input"), self._pad():
    ...
    def _section_radio(self) -> None:
        self._section_header("Radio", "Single-selection radio button groups.", "radio")
        with Grid(cols=2, gap=5):
            with self._card_wrap("Radio Group", "3 options"), self._pad():
                Radio(
                    options=[
                        ("free", "Free — $0/month"),
                        ("pro", "Pro — $12/month"),
            ...
            with self._card_wrap("Radio Group", "4 options"), self._pad():
                Radio(
                    options=[
                        ("web", "Web"),
                        ("desktop", "Desktop"),
    ...
    def _section_forms(self) -> None:
        self._section_header(
            "Forms", "Structured form containers with labels and validation.", "forms"
        )
        with Grid(cols=2, gap=5):
    ...
    def _section_alerts(self) -> None:
        self._section_header(
            "Alerts", "Inline status messages with left-accent border design.", "alerts"
        )
        with Flex(direction="col", gap=3):
            ...
            Alert("Success", "Your changes have been saved successfully.", variant="success")
            Alert(
                "Warning",
                "Your trial expires in 3 days. Upgrade to keep access.",
                variant="warning",
            )
            Alert("Error", "Failed to connect to the server. Please try again.", variant="danger")
    ...
    def _section_overlays(self) -> None:
        self._section_header(
            "Overlays", "Modal, Drawer, and Tooltip — focused interaction layers.", "overlays"
        )
        with Grid(cols=2, gap=5):
    ...
    def _section_loading(self) -> None:
        self._section_header(
            "Loading States", "Spinner, Progress, and Skeleton for async content.", "loading"
        )
        with Grid(cols=3, gap=5):
            ...
            # Progress bars
            with self._card_wrap("Progress", "0 – 100%"), self._pad():
                with Flex(direction="col", gap=3):
            ...
            # Skeleton
            with self._card_wrap("Skeleton", "shimmer"), self._pad():
                with Flex(align="center", gap=3):
    ...
    def _section_nav(self) -> None:
        ...
        with Flex(direction="col", gap=5):
            ...
            with self._card_wrap("Breadcrumb", "path navigation"), self._pad():
                Breadcrumb(
                    items=[
                        ("Home", "/"),
    ...
    def _section_pagination(self) -> None:
        self._section_header(
            "Pagination", "Page controls for navigating large datasets.", "pagination"
        )
        with self._card_wrap("Pagination", "current / total"), self._pad():
    ...
    def _section_menu(self) -> None:
        self._section_header(
            "Menu", "Contextual dropdown menus for actions and navigation.", "menu"
        )
        with Grid(cols=3, gap=5):
    ...
    def _section_stats(self) -> None:
        self._section_header("Stats", "Key metric cards with trend indicators.", "stats")
    ...
    def _section_tables(self) -> None:
        self._section_header(
            "Tables", "Structured data grids with striped rows and hover states.", "tables"
        )
        with Flex(direction="col", gap=5):
    ...
    def _section_charts(self) -> None:
        self._section_header("Charts", "Line, Bar, and Pie charts powered by Chart.js.", "charts")
        with Grid(cols=2, gap=5):
        ...
        with self._card_wrap("Markdown", "marked.js"), self._pad():
...
def run_storybook(port: int = 8000, open_browser: bool = True) -> None:
    ...
    # Get the path to this storybook.py file for hot reload
    frame = inspect.currentframe()
    storybook_file = inspect.getfile(frame) if frame is not None else __file__

    run_dev_server(StorybookApp, port=port, open_browser=open_browser, watch_file=storybook_file)
